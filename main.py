...
class BotDCAApp(ctk.CTk):
    def __init__(self):
        ...
        self.after(3000, self.recuperar_bots_del_servidor)
    ...
    def recuperar_bots_del_servidor(self):
        try:
            ...
            for bot_id in bot_ids:
                ...
                if exchange_id not in self.exchange_instances:
                    print(
                        f"[WARNING] Exchange {exchange_id} no conectado aún.")
                    continue
            ...
            print(f"[ERROR] Al recuperar bots del servidor: {e}")
    ...
    class ServerSync:
        ...
        def start_sync(self):
            try:
                ...
                self.send_bots()
                while True:
                    time.sleep(10)  # cada 10 segundos sincroniza
                    self.send_bots()
            except Exception as e:
        ...
        def send_bots(self):
            try:
                ...
                for bot in self.app.bots:
                    if not hasattr(bot, "exchange_id"):
                        print("[BOTS WARNING] Bot sin atributo 'exchange_id'")
                        continue
                ...
                requests.post(f"{self.url}/api/report_bots", json=bots_data)
